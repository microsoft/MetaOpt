...
namespace MaxConcurrentFlow
{
    ...
    public class WorkerThread
    {
        ...
        public WorkerThread
            (YoungsAlgorithm y, int _tid)
        {
            threadId = _tid;

            yts = new Y_ThreadState();
        ...
        public void CopyMinPaths(SharedState ss, bool singleThread=false)
        {
            sw = new Stopwatch();
            sw.Start();
            this.ss = ss;
        ...
        public void RunOnPartition()
        {
            ...
            while(iterations < HowManyIters)
            {
                ...
                for (int j = 0; j < yetToSatisfyReqs.Count; j++)
                {
                    ...
                    try
                    {
                        ss.rwl.AcquireReaderLock(WorkerThread.ReadTimeoutMS);
                ...
                try
                {
                    ss.rwl.AcquireWriterLock(WorkerThread.WriteTimeoutMS);
    ...
    public class SharedState
    {
        ...
        public SharedState(YoungsAlgorithm y, double _beta)
        {
    ...
    public class YoungsAlgorithm_mt
    {
        // all inputs are here
        YoungsAlgorithm y;
        ...
        public YoungsAlgorithm_mt(YoungsAlgorithm _y, int _numThreads)
        {
            y = _y;
            numThreads = _numThreads;
        }

        public double CheckFeasibility(double beta)
        {
            ...
            {
                workers[i] = new WorkerThread(y, i);
            }
            ss = new SharedState(y, beta);
