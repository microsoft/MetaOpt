...
{
    public class S14_Y_ThreadState
    {
        //
    ...
    public class S14_WorkerThread
    {
        static int ReadTimeoutMS = 1000, WriteTimeoutMS = 3000;
        public static int HowManyIters = 1000;
        Stopwatch sw;

        S14_Y_ThreadState yts;
        S14_SharedState ss;
        public int threadId;
        ...
        public S14_WorkerThread
            (S14_YoungsAlgorithm y, int _tid)
        {
            threadId = _tid;

            yts = new S14_Y_ThreadState();
        ...
        public void CopyMinPaths(S14_SharedState ss, bool singleThread = false)
        {
            sw = new Stopwatch();
            sw.Start();
            this.ss = ss;
        ...
        public void RunOnPartition()
        {
            ...
            while (iterations < HowManyIters)
            {
                ...
                int istar = 0, jstar = 0;
                double old_sumy = 0, old_sumz = 0, new_sumY = 0;
                ...
                for (int j = 0; j < yetToSatisfyReqs.Count; j++)
                {
                    ...
                    try
                    {
                        ss.rwl.AcquireReaderLock(S14_WorkerThread.ReadTimeoutMS);
                ...
                // prepare updates to y(e), flow, sumY
                ...
                try
                {
                    ss.rwl.AcquireWriterLock(S14_WorkerThread.WriteTimeoutMS);
    ...
    public class S14_SharedState
    {
        // read only
        public Request[] requests;
        public double epsilon, alpha, beta;
        public int m, T, E;
        ...
        public S14_SharedState(S14_YoungsAlgorithm y, double _alpha, double _beta)
        {
    ...
    public class S14_YoungsAlgorithm_mt
    {
        // all inputs are here
        S14_YoungsAlgorithm y;

        // thread local state
        S14_WorkerThread[] workers;
        int numThreads;

        S14_SharedState ss;

        public S14_YoungsAlgorithm_mt(S14_YoungsAlgorithm _y, int _numThreads)
        {
            y = _y;
        ...
        public double CheckFeasibility
            (double alpha, double beta, out double achieved_alpha)
        {
            // partition the work, issue the threads

            workers = new S14_WorkerThread[numThreads];
            for (int i = 0; i < numThreads; i++)
            {
                workers[i] = new S14_WorkerThread(y, i);
            }
            ss = new S14_SharedState(y, alpha, beta);
            ...
            do
            {
                ...
                Console.WriteLine("Scatter iter {0} = {1}ms {2}#req_remain progress {3}T {4}I",
                    scatter_iter - 1, 
                ...
                foreach (S14_WorkerThread w in workers)
                {
