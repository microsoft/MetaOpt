...
class SpectralClustering(AbstractPartitioningMethod):
    ...
    def run_k_means_on_eigenvectors(self, eigvecs, num_nodes):
        ...
        k_means = KMeans(self._num_partitions, n_init=100, random_state=self.seed).fit(U)
    ...
    def _partition_impl(self, G, topo_name=""):
        ...
        if not hasattr(self, '_num_partitions'):
            ...
            print('Using eigengap heuristic to select number of partitions, max: {}'.format(max_num_parts))
            self.eigengaps = np.array([eigvals[i+1] - eigvals[i] for i in range(len(eigvals[:max_num_parts]) - 1)])
            ...
            while k < len(indices):
                ...
                if all_partitions_contiguous(G, p_v):
                    break
                k += 1
            if k == len(indices):
                raise Exception('could not find valid partitioning')

            print('Eigengap heuristic selected {} partitions'.format(self._num_partitions))
            return p_v
