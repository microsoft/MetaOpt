...
namespace MetaOptimize.Test
{
    ...
    public class PopEncodingTests
    {
        ...
        /// </summary>
        [TestMethod]
        public void TestPopGapSimpleWithGurobi()
        {
            var topology = new Topology();
            topology.AddNode("a");
            topology.AddNode("b");
            ...
            partition.Add(("a", "b"), 0);
            partition.Add(("b", "a"), 1);
            var popEncoder = new PopEncoder<GRBVar, SolverGuroubi>(() => new SolverGuroubi(), topology, k: 1, numPartitions: 2, demandPartitions: partition);
            var encoding = popEncoder.Encoding();
            var solverSolution = encoding.Solver.Maximize(encoding.MaximizationObjective);
            var optimizationSolution = popEncoder.GetSolution(solverSolution);

            var solver = (SolverGuroubi)encoding.Solver;
            /*
            // Debugging information.
            foreach (var c in solver.ConstraintExprs)
            {
